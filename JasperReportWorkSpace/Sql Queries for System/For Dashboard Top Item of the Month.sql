select pname, sum(withoutTaxSales) as withouttaxsale , sum(salesqty) as salesqty from
...
having sum(sal.sales_qty) > 0 order by ref.refund_date,cat.cat_name,(sal.sales_qty) desc)
 ) as t where tdate between '2020-10-01' and '2020-10-30' group by prdid having sum(salesqty) > 0 order by salesqty desc limit 10;