select user,catname as category ,prdid as productid, pname as productname,sum(salesqty) as salesqty, sum(TotalSales) as totalSales, 
...
having sum(sal.sales_qty) > 0  order by  ref.refund_date,cat.cat_name,(sal.sales_qty) desc)
 ) as t where tdate between '2020-10-01' and '2020-10-31' group by user,catname,prdid having sum(salesqty) > 0 order by catname,salesqty ;