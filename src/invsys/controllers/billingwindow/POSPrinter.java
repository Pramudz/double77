...
//this is the main printer class where POS bill printing is done
class POS {
	
	
	//kanishka 2023-02-09
		public static void printDoMax(byte [] prm_message, String prm_port) {
			try {
				
				byte data[] = prm_message;
				FileOutputStream fos;
				
				fos = new FileOutputStream(prm_port);
				 fos.flush();
	...
	public static class POSPrinter {
		...
		public static class QrCode {
			...
			public static byte[] Print(String qrData, ErrorsCorrection err, QrCodeSize size) {
				ByteArrayOutputStream bytes = new ByteArrayOutputStream();
				bytes.writeBytes(QrCode.ModelQr());
				bytes.writeBytes(QrCode.SizeQr());
				bytes.writeBytes(QrCode.ErrorQr(ErrorsCorrection.Percent7));
				bytes.writeBytes(QrCode.StoreQr(qrData));
				bytes.writeBytes(qrData.getBytes());
				bytes.writeBytes(QrCode.PrintQr());
				return bytes.toByteArray();
			}
